...
#Notes:
# the 'something good happened' neuron is neuron # -1 
# the 'something bad happened' neuron is neuron # -2 

# FINISH the 'badthinghappened' method
...
class Neuron: 
	...
	def activateOutgoingNeurons(self): #if it's newly activated it does this
		for key,level in self.outgoingNeuronsConnectedTo.items():
			...
			#print "debug"
			#print neuronList[key].incomingNeuronsActivated
...
###################################################################################################################
	...
	def weakenConnections(self):
		pass #increase threshhold to forget things
...
def createGoodNeuron(neuronnumber): #determine what kind of neuron gets called 'Good'
	...
	if neuronList[neuronnumber].topOfTheChain == 1: 
		if -1 in neuronList[neuronnumber].incomingNeuronsConnectedTo.keys(): #right now, -1 active is 'good' but it could be anything
			if neuronList[neuronnumber].good == 0:
				neuronList[neuronnumber].good = 1
	
	
#####################################################################################################
# BAD BAD BAD BAD BAD BAD BAD BAD BAD BAD BAD BAD BAD BAD BAD BAD BAD BAD BAD BAD BAD BAD 

def badThingHappened(): #make incoming neurons bad #NEED TO PUT THIS IN TAC TOE PROGRAM #NEED TO PUT THIS IN TAC TOE PROGRAM#NEED TO PUT THIS IN TAC TOE PROGRAM#NEED TO PUT THIS IN TAC TOE PROGRAM
	if self.value == -2: #the BAD neuron
		if self.neuronCurrentActivationLevel > 0:
			for key in self.incomingNeuronsConnectedTo.keys():
				neuronList[key].bad = 1
	# the top of the chain below it is a warning 
	# LIST OF BAD NEURONS FROM TOP OF CHAIN AT A LOST GAME? neuronnumber.bad = 1
	

def badIncomingPartiallyActivated(): #NEED TO PUT THIS IN MAIN LOOP#NEED TO PUT THIS IN MAIN LOOP#NEED TO PUT THIS IN MAIN LOOP#NEED TO PUT THIS IN MAIN LOOP#NEED TO PUT THIS IN MAIN LOOP
	if self.neuronActive == 0:
		if self.bad == 1:		
			if self.neuronCurrentActivationLevel > 0:
				#find largest from 1-9 (inactive), or a random one
	#if bad partiallly activated, try a random thing, or try a good alternative
		#check if omoves 1-9 are partially activated 0, O is #10
		#try a random thing
		

# AVOIDING A BAD THING IS GOOD
def checkIfaBadThingWasAvoided(): #this is activated in the deactivate method
	# a half activated bad thing that gets deactivated without being activated, is a good thing
	if self.bad == 1:
		if self.neuronActive == 0: #it didn't get fully activated
			activateANewNeuronMethod(-1) #activate the good neuron
		

	#next time it must value that good thing and activate it
...
for index in range(-2,len(neuronList)):
...
#eventually backup memory to a text file
